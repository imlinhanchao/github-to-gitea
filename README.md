...
4. 项目启动后，通过网页进行初始化配置（GitHub Token、Gitea Token、管理员账号密码、MySQL 连接信息），配置保存到 `config.json`。
...
> **注意**：后端使用 MySQL 作为数据存储。请确保 MySQL 服务已运行，并提前创建好对应数据库（如 `github_to_gitea`）。  
> 修改 MySQL 连接信息后需**重启后端**才能生效。  
> 首次启动（`config.json` 不存在）时，后端会临时使用内存 SQLite，保证配置页面可正常访问。
